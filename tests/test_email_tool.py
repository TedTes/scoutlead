...
def test_gmail_email_provider_refreshes_token_and_sends_mime_message(monkeypatch) -> None:
    ...
    with session_factory() as session:
        workspace_id = "workspace:gmail-tool"
        session.add(WorkspaceModel(id=workspace_id, name="Gmail tool workspace"))
        session.commit()
...
def test_gmail_email_provider_reports_missing_account_connection() -> None:
    engine = create_engine("sqlite:///:memory:", connect_args={"check_same_thread": False})
    create_database(engine)
    session_factory = sessionmaker(bind=engine, expire_on_commit=False)
    now = datetime.now(timezone.utc)
    ...
    with session_factory() as session:
        with pytest.raises(ConfigurationError) as exc_info:
            ...
            ).bind_session(session).send(product=product, lead=lead, message=message)

    assert exc_info.value.message == "Gmail is not connected for this account"
    assert exc_info.value.details["workspace_id"] == "workspace:gmail-tool"
    assert exc_info.value.details["user_message"] == "Connect Gmail in Integrations before sending outreach."
